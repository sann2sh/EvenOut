...
This is the Node.js/NestJS monolithic backend for the EvenOut application. It serves as the core engine that handles authentication, user profiles, group expense ledgers, push notifications, receipt OCR parsing, and mathematical debt simplification.
...
### Update My Profile
- **URL:** `PATCH /users/me`
- **Function:** Updates the user's profile details (partial or full updates).
- **Request Body:**
  ```json
  {
    "username": "sann2sh",
  ...
  ```
- **Notes:** 
  - `username` rules: 3–50 chars, only letters, numbers, underscores. Returns `409 Conflict` if taken.
  - You can perform partial updates by only sending the fields you wish to change.
...
### Create Expense
- **URL:** `POST /expenses`
- **Function:** Records a new expense. For Offline-First, the frontend generates and passes the `id`. For P2P expenses (no group), omit the `group_id`. Optionally include `receipt_url` and `parsed_items`.
- **Request Body:**
  ```json
  {
    "id": "generated-uuid-by-client", 
...
### Parse Receipt via Image URL
- **URL:** `POST /expenses/parse-receipt`
- **Function:** Downloads a public image URL and passes it to the OCR API to extract structured JSON line items (Item, Qty, Price).
- **Request Body:**
  ```json
  {
    "imageUrl": "https://..."
...
- **Function:** Gets the raw total balance of each user in a group.
- **Response:**
  ```json
  [
    {
...
### Trigger Push Notification Nudge
- **URL:** `POST /nudges/send`
- **Function:** Allows a user to manually trigger a quirky push notification to someone who owes them money. Validates debt exists before dispatching FCM payload.
- **Request Body:**
  ```json
  {
