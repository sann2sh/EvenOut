...
PATCH /api/v1/users/me — Now also accepts username
```
json
{
  "username": "sann2sh",
  "display_name": "Sann",
  "phone_number": "+977980000000",
  "avatar_url": "https://..."
}
```
Username rules: 3–50 chars, only letters, numbers, underscores (sann_2sh ✅, sann 2sh ❌)
If username is already taken, returns 409 Conflict with a clear error message.


GET /api/v1/users/search?query=sann — New search endpoint

Searches by both username AND display_name (case-insensitive).
Excludes yourself from results.
Returns up to 20 matches: [{ id, username, display_name, avatar_url }]
